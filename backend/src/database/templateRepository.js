...
async function getAllByUser(userId) {
    return new Promise((resolve, reject) => {
        db.query(`SELECT * FROM templates WHERE user_id = ?`, [userId], (err, results) => {
            ...
            const templates = results.map(t => new Template(t.id, t.user_id, t.name, t.canvas_json));
            resolve(templates);
        });
    });
}
    ...
    return new Promise((resolve, reject) => {
        db.query(
            ...
            (err, results) => {
                ...
                const t = results[0];
                resolve(new Template(t.id, t.user_id, t.name, t.canvas_json));
            }
        );
    });
}
...
async function updateTemplate(template) {
    return new Promise((resolve, reject) => {
        db.query(
            ...
            [template.name, template.canvas_json, template.id, template.user_id],
            (err, results) => {
                if (err) return reject(err);
